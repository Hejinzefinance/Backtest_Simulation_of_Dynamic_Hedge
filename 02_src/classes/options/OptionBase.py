...
#%% 初始化
    ...
    def __init__(self):
        self.reset_paras()
        self.greek_columns = ['sigma','left_days','left_times','sigma_T','stock_price','d1','nd1','Nd1','Nd2','delta','gamma','vega','theta','option_price','cash_delta','cash_gamma','cash_theta','option_value']
    ...
    def set_paras(self,notional=None,start_date=None,end_date=None,K=None,r=None,option_fee=None,stock_code=None,start_price=None):
    ...
    def calculate_trade_dates(self):
        ...
        self.trade_datetimes = sorted([datetime(x.year,x.month,x.day,9,30) for x in self.trade_dates]+[datetime(x.year,x.month,x.day,15) for x in self.trade_dates])
        self.look_back_datetimes = sorted([datetime(x.year,x.month,x.day,9,30) for x in self.look_back_dates]+[datetime(x.year,x.month,x.day,15) for x in self.look_back_dates])
